...
client.connect((err) => {
  if (err) {
    ...
    console.log(
      `Database => ${client.database}, Port => ${client.port}, connected by => ${client.user}`
    );
  }
});
  ...
  form.parse(req, async (err, fields: any, files: any) => {
    ...
    if (Object.keys(files).length === 0) {
      ...
      for (let tag of eachTag) {
        if (tag !== "") {
          ...
          if (!result.rows.length) {
            client.query("insert into tags (name) values ($1) returning id", [
              tag,
            ]);
            let result = await client.query(
              "select id from tags where name = $1",
      ...
      if (req.session.user) {
        ...
        let result = await client.query(
          "insert into post (title,content,created_at,updated_at, users_id) values ($1,$2,now(),now(),$3) returning id;",
        ...
        let result = await client.query(
          "insert into post (title,content,created_at,updated_at) values ($1,$2,now(),now()) returning id;",
      ...
      if (req.session.user) {
        ...
        let result = await client.query(
          "insert into post (title,content,image,created_at,updated_at, users_id) values ($1,$2,$3,now(),now(),$4) returning id;",
          [input.title, input.content, input.image, userID]
        );
        post_id = result.rows[0].id;
        ...
        let result = await client.query(
          "insert into post (title,content,image,created_at,updated_at) values ($1,$2,$3,now(),now()) returning id;",
      ...
      for (let tag_id of tags_id) {
        await client.query(
          "insert into post_tag (post_id, tags_id) values ($1,$2)",
          [post_id, tag_id]
        );
      }
...
app.get("/tags/:id", async (req, res) => {
  ...
  // console.log(id)
  let result = await client.query(
    "select post.id, post.title, post.content, post.created_at, post.image, tags.name from post inner join post_tag on post.id = post_tag.post_id inner join tags on tags.id = post_tag.tags_id where post.id = $1;",
    [id]
  );
...
app.post("/main", async (req, res) => {
  ...
  let result = await client.query(
    "select * from post order by created_at desc offset $1 limit 8",
    [contentIndex]
  );
...
app.get("/post/:id", async (req, res) => {
  ...
  if (result.rows.length > 0) {
    ...
    let result = await client.query(
      "select id,title,content,image from post where id = $1",
      [id]
    );
    let posts = result.rows[0];
...
app.patch("/post/:id", adminGuard, (req, res) => {
  ...
  client
    ...
    .then((result) => {
      res.json({ ok: true });
      return client.query(/* sql */ `select image from post where id = $1`, [
        id,
      ]);
    })
    .then((result) => {
...
app.delete("/post/:id", adminGuard, (req, res) => {
  ...
  client
    ...
    .then(() => {
      return client.query(/*sql*/ `delete from post where id = $1`, [id]);
    })
    .then((result) => {
      if (result.rowCount) {
        res.json({ ok: true });
...
app.get("/search", async (req, res) => {
  let result = await client.query(
    "select rank() over (order by count(*) desc),count(*), tags.name from tags inner join post_tag on tags.id = post_tag.tags_id group by tags.name order by count(*) desc limit 5;"
  );
  // console.log(result)
  res.json({ result });
});
